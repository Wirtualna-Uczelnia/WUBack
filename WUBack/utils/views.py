...
def get_change_group_requests(request):
    ...
    sf_response = requests.get(instance_url + f"/services/data/v50.0/query/?q=SELECT+Id,From_Group__c,To_Group__c,Who__c+FROM+Group_Change_Request__c+WHERE+From_Group__c='{from_didactic_group_id}'+AND+Who__c+IN+(SELECT+Id+FROM+Didactic_Group_Member__c+WHERE+Login__c='{login}')", headers={
                               "Authorization": "Bearer "+access_token}).json()
